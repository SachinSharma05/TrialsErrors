...
Public Class frmStockOut
    ...
    Private Function GenerateID() As String
        ...
        Try
            ...
            con.Open()
            cmd = New OleDbCommand("SELECT TOP 1 ID FROM StockTransfer ORDER BY ID DESC", con)
    ...
    Private Sub ComboBox4_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ComboBox4.SelectedIndexChanged
        ...
        myConnection.Open()
        Dim str As String
        str = "SELECT Stock FROM ItemMaster WHERE (Prod_Name = '" & ComboBox4.Text & "')"
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        Try
            ...
            If TextBox3.Text = "" Then
                MessageBox.Show("Please Enter Location of Transfer", "", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                TextBox3.Focus()
                Exit Sub
            End If
            If TextBox4.Text = "" Then
                MessageBox.Show("Please Enter Quantity to Transfer", "", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                TextBox4.Focus()
                Exit Sub
            End If
            If Val(TextBox4.Text) > Val(TextBox5.Text) Then
                MessageBox.Show("Transfer Qty is more than Present Stock", "", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                TextBox4.Clear()
                TextBox4.Focus()
                Exit Sub
            End If
            ...
            myConnection.Open()
            Dim str As String
            str = "insert into StockTransfer ([OutID], [OutType], [OutCategory], [OutSubCat], [OutProdName], [Person], [Location], [OutQty], [OutDate], [OutReason]) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
            Dim cmd As OleDbCommand = New OleDbCommand(str, myConnection)
            cmd.Parameters.Add(New OleDbParameter("OutID", CType(TextBox1.Text, String)))
            cmd.Parameters.Add(New OleDbParameter("OutType", CType(ComboBox1.Text, String)))
            cmd.Parameters.Add(New OleDbParameter("OutCategory", CType(ComboBox2.Text, String)))
            cmd.Parameters.Add(New OleDbParameter("OutSubCat", CType(ComboBox3.Text, String)))
            cmd.Parameters.Add(New OleDbParameter("OutProdName", CType(ComboBox4.Text, String)))
            cmd.Parameters.Add(New OleDbParameter("Person", CType(TextBox2.Text, String)))
            cmd.Parameters.Add(New OleDbParameter("Location", CType(TextBox3.Text, String)))
            cmd.Parameters.Add(New OleDbParameter("OutQty", CType(TextBox4.Text, String)))
            cmd.Parameters.Add(New OleDbParameter("OutDate", CType(DateTimePicker1.Value.Date, String)))
            cmd.Parameters.Add(New OleDbParameter("OutReason", CType(ComboBox5.Text, String)))
            cmd.ExecuteNonQuery()
            cmd.Dispose()
            myConnection.Close()
            MsgBox("Stock Transferred Successfully", vbOKOnly)
